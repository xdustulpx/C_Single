...
int main(void){
    ...
    /*
    bits=(int)log10((double)N)+1;//求整数N的十进制位数（BigNat类型数组的位数）
    initialisation(LG_MAX,n);//初始化n数组
    initialisation(LG_MAX,fac);//初始化fac数组
    lire(N,bits,n);//将int型N转化为BigNat型n
    */
...
int factorielle(int n,BigNat fac){
    ...
    fac[LG_MAX]=0;
    BigNat i,n1;
    initialisation(LG_MAX,i);
    initialisation(LG_MAX,n1);
    k_i=calculate_bits(i);
    lire(1,1,n1);
    lire(1,1,fac);
    len=calculate_bits(fac);
    for(;comparaisons(k_i,k,i,n,"<");){
        additionner(k_i,1,i,n1,i);
        k_i=calculate_bits(i);
        len=multiplier(len,k,fac,n,fac);
    }
    return len;
    **/
}
